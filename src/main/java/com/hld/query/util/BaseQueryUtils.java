...
public class BaseQueryUtils {
    ...
    public static List<IFilter> splitFilterNameAlias(List<IFilter> filters, List<TableInfo> tableInfos) {
        ...
        if (tableInfos == null || tableInfos.size() <= 0) {
            throw new RuntimeException("params tableInfos is not null or empty");
        }
            ...
            if (filter.getFilterType() == FilterType.OR_NEW || filter.getFilterType() == FilterType.AND_NEW) {
                ...
                if (StringUtils.isNotBlank(filterName)) {
                    for (int j = 0; j < size; j++) {
                        ...
                        if (j == size - 1) {
                            throw new RuntimeException("无此过滤参数：" + filterName);
                        }
                    }
     ...
     * @return
     */
    public static String splitWhereSql(String firstSql, String sql, Map<String, Object> paramValues, List<IOrderBy> orderBys, List<IFilter> filters, List<String> groupBys) {
        if (StringUtils.isEmpty(sql) || paramValues == null) {
    ...
    public static List<IOrderBy> splitOrderBys(List<IOrderBy> orderBys, List<TableInfo> tableInfos) {
        ...
        if (tableInfos == null || tableInfos.size() == 0) {
            throw new RuntimeException("params error : tableInfos is not null or empty");
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (StringUtils.isEmpty(orderByName)) {
                throw new RuntimeException("排序参数字段名称不能为空");
            }
            ...
            for (int j = 0; j < size; j++) {
                ...
                if (j == size - 1) {
                    throw new RuntimeException("无此排序参数：" + orderByName);
                }
            }
        }
        return orderBys;
    ...
    public static List<String> splitGroupBys(List<String> groupBys, List<TableInfo> tableInfos) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                if (j == size - 1) {
                    throw new RuntimeException("无此分组列：" + groupByName);
                }
            }
        }
        return groupBys;
            ...
            for (int i = 0; i < classAnnotations.length; i++) {
                ...
                if (annotation instanceof TableRelations) {
                    ...
                    if (StringUtils.isEmpty(tableRelations)) {
                        throw new RuntimeException("注解参数：{relation} 不能为空");
                    }
                ...
                if (i == classAnnotations.length - 1) {
                    throw new RuntimeException(c.getName() + "需添加表间关系注解 TableRelations");
                }
            }
        } else {
            throw new RuntimeException(c.getName() + "需添加表间关系注解 TableRelations");
            ...
            for (Annotation annotation : annotations) {
                if (annotation instanceof TableFiledInfo) {
                    ...
                    if (StringUtils.isEmpty(tableAlia) && StringUtils.isEmpty(mTableAlias)) {
                        throw new RuntimeException("table alias 别名不能为空");
                    }
        ...
        if (classAnnotations.length > 0) {
            for (int i = 0; i < classAnnotations.length; i++) {
                ...
                if (annotation instanceof TableRelations) {
                    ...
                    if (StringUtils.isEmpty(tableRelations)) {
                        throw new RuntimeException(c.getName() + "需添加表间关系参数");
                    }
